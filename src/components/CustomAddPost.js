...
function CustomAddPost({
  onAddPost,
  currentCatImage,
  setCurrentCatImage,
  usedImages,
}) {
  const [customTitle, setCustomTitle] = useState('');  // Используем другое имя для состояния
    ...
    axios.get('https://cataas.com/cat')
      .then(response => {
        ...
        if (!usedImages.includes(newImage)) {
          setCurrentCatImage(newImage); // Обновляем изображение в App
          const newPost = {
            id: Date.now(),
            title: customTitle,  // Используем другое имя для состояния
            description: '',
            image: newImage,
          };
